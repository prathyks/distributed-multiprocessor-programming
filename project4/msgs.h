#include"sem.h"
#define SIZE_MSG 10
#define SIZE_PORT_BUFFER 20
#define NUM_PORTS 100

typedef struct msg_t{
	int replyport; // address of the sender
	int data[SIZE_MSG]; //message data
} msg_t;
...
void send(int dest, msg_t* message){
	...
	P(dest_port->full);
	P(dest_port->mutex);
	memcpy(&(dest_port->msg_buffer[dest_port->in]),message,sizeof(msg_t));
	dest_port->in = (dest_port->in+1) % SIZE_PORT_BUFFER;
	V(dest_port->mutex);
	V(dest_port->empty);
}

void receive(int src, msg_t* result){
